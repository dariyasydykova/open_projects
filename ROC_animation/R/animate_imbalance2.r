...
outcome1 <- data.frame(predictor = rnorm(50, mean = -5, sd = 5), outcome = 1, time = 2)
outcome2 <- outcome2 %>% mutate(time = 2)
...
p_dist_gif <- animate(p_dist,
                      ...
                      width = 400,
                      height = 400,
                      renderer = file_renderer("./gganim_imbalance2", prefix = "p_dist", overwrite = TRUE))
p_ROC_gif <- animate(p_ROC,
                     device = "png",
                     width = 400,
                     height = 400,
                     renderer = file_renderer("./gganim_imbalance2", prefix = "p_ROC", overwrite = TRUE))
p_PR_gif <- animate(p_PR,
                    device = "png",
                    width = 400,
                    height = 400,
                    renderer = file_renderer("./gganim_imbalance2", prefix = "p_PR", overwrite = TRUE))
...
# save as gif
image_write(combined_gif, "imbalance2.gif")
