...
outcome1 <- data.frame(predictor = rnorm(10, mean = -5, sd = 5), outcome = 1, time = 1, n = "100")
outcome2 <- data.frame(predictor = rnorm(90, mean = 5, sd = 5), outcome = 2, time = 1, n = "100")
outcome1 %>% rbind(outcome2) -> df_t1

# two classes are imbalanced at time 2
outcome1 <- data.frame(predictor = rnorm(100, mean = -5, sd = 5), outcome = 1, time = 2, n = "1,000")
outcome2 <- data.frame(predictor = rnorm(900, mean = 5, sd = 5), outcome = 2, time = 2, n = "1,000")
outcome1 %>% rbind(outcome2) -> df_t2

outcome1 <- data.frame(predictor = rnorm(1000, mean = -5, sd = 5), outcome = 1, time = 3, n = "10,000")
outcome2 <- data.frame(predictor = rnorm(9000, mean = 5, sd = 5), outcome = 2, time = 3, n = "10,000")
outcome1 %>% rbind(outcome2) -> df_t3

outcome1 <- data.frame(predictor = rnorm(10000, mean = -5, sd = 5), outcome = 1, time = 4, n = "100,000")
outcome2 <- data.frame(predictor = rnorm(90000, mean = 5, sd = 5), outcome = 2, time = 4, n = "100,000")
outcome1 %>% rbind(outcome2) -> df_t4

# combine the two datasets
rbind(df_t1, df_t2, df_t3, df_t4) -> anim_data
...
p_dist <- ggplot(anim_data) +
  ...
  geom_rug(aes(x = predictor, color = factor(outcome)), alpha = 0.3, sides="b") + 
...
p_dist_gif <- animate(p_dist,
                      ...
                      width = 400,
                      height = 400,
                      renderer = file_renderer("../animations/gganim_imbalance3", prefix = "p_dist", overwrite = TRUE)
)
p_ROC_gif <- animate(p_ROC,
                     device = "png",
                     width = 400,
                     height = 400,
                     renderer = file_renderer("../animations/gganim_imbalance3", prefix = "p_ROC", overwrite = TRUE)
)
p_PR_gif <- animate(p_PR,
                    device = "png",
                    width = 400,
                    height = 400,
                    renderer = file_renderer("../animations/gganim_imbalance3", prefix = "p_PR", overwrite = TRUE)
)
